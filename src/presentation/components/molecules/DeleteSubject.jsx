...
export default function DeleteSubject({
  ...
  const { grade: gradeId, area: areaId } = useParams();
  const navigate = useNavigate();
  const deleteSubject = () => {
    services.subjects.remove(setIsLoading, subjectId).then(() => {
      if (setSubjectActive) {
        setSubjectActive({});
        refresh();
      } else {
        ...
        <div className='flex flex-col h-full w-full gap-2'>
          <div className='flex flex-col gap-2 mb-6 justify-center items-center'>
            ...
            <span className='text-sm text-gray-300'>
              Estás apunto de eliminar una asignatura, si lo haces perderás toda
              la información relacionada a esta como objetivos, competencias,
