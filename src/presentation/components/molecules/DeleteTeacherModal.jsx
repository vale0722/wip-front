...
export default function DeleteTeacherModal({
  ...
  const handleDeleteTeacher = () => {
    deleteTeacher(teacherActive.id);
    if (setTeacherActive) {
      setTeacherActive({});
      refresh();
    } else {
      navigate(config.routes.auth.teachers.path, true);
    }
  };
  return (
    <div id='confirm-delete' className='modal modal-bottom sm:modal-middle'>
        ...
        <div className='flex flex-col h-full w-full gap-2'>
          <div className='flex flex-col gap-2 mb-6 justify-center items-center'>
            ...
            <span className='text-sm text-gray-300'>
              Estás apunto de eliminar un(a) profesor(a), si lo haces perderás
              toda la información relacionada al profesor:
              {teacherActive ? teacherActive.name : ''}
            </span>
          </div>
